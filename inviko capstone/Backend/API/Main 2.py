...
def register():
	...
	cursor.execute('''INSERT INTO user (First_Name, Last_Name, Email, Password, Address1, Address2, City, Postalcode, Province, Height, Weight, Age, Contact_Number,Role) VALUES
					(%s, %s, %s, %s, %s, %s)''',(Firstname,Lastname,Email,Password, Address1, Address2, City, Postalcode, Province, Height, Weight, Age, Contactnumber, Role))
...
def list_appointment():
    try:
        ...
        return json.dumps({"status":200,"List_Appointment":list_appointment})
    except Exception as e:
...
def add_diagnosis():
	...
	diagnosis = request.json.get('Diagnosis_id')
	prescription = request.json.get('Prescription')
	report = request.json.get('Report')
	
	cursor.execute('''INSERT INTO diagnosis (doctor_id, patient_id, appointment_id, diagnosis, prescription, report) VALUES
